...
const { tarkov } = require('../services/unifiedPatchScheduler');
const { forceGetLatestPatch, loadLastPatch } = tarkov;
const { sendPatchToChannel, setPatchChannel, removePatchChannel, getPatchChannel } = tarkov;

module.exports = {
  data: new SlashCommandBuilder()
    .setName('타르코프패치노트')
    .setDescription('Escape from Tarkov 패치노트 관련 명령어')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addSubcommand((sub) =>
      sub
        .setName('최신')
        .setDescription('최신 타르코프 패치노트를 AI로 요약해서 보여줍니다')
    )
    .addSubcommand((sub) =>
      sub
        .setName('설정')
        .setDescription('타르코프 패치노트 자동 알림 채널을 설정합니다')
        .addChannelOption((opt) =>
          opt
            .setName('채널')
            .setDescription('타르코프 패치노트를 받을 채널')
            .setRequired(true)
    ...
    .addSubcommand((sub) =>
      sub
        .setName('해제')
        .setDescription('타르코프 패치노트 자동 알림을 해제합니다')
    )
    .addSubcommand((sub) =>
      sub
        .setName('상태')
        .setDescription('타르코프 패치노트 알림 상태를 확인합니다')
    ),
    ...
    try {
      ...
      if (!patchData) {
        return interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setTitle('❌ 타르코프 패치노트를 가져올 수 없습니다')
              .setDescription('타르코프 패치노트 페이지에 접근할 수 없습니다.\n잠시 후 다시 시도해주세요.')
              .setColor(0xff0000),
          ],
        });
      }
      ...
      await interaction.editReply({
        content: '✅ 타르코프 패치노트 요약이 완료되었습니다!',
        embeds: [],
      });

      await sendPatchToChannel(interaction.channel, patchData);
    } catch (err) {
      console.error('타르코프 패치노트 명령어 오류:', err);
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setTitle('❌ 오류 발생')
            .setDescription(`타르코프 패치노트를 가져오는 중 오류가 발생했습니다.\n${err.message}`)
            .setColor(0xff0000),
        ],
      });
    }
  ...
  async setChannel(interaction) {
    ...
    await interaction.reply({
      embeds: [
        new EmbedBuilder()
          .setTitle('✅ 타르코프 패치노트 알림 채널 설정 완료')
          .setDescription(
            `${channel} 채널에 타르코프 패치노트 알림이 전송됩니다.\n\n` +
            '새로운 패치가 나오면 이 채널에 자동으로 알림이 옵니다!\n' +
            '해제하려면 `/타르코프패치노트 해제`를 사용하세요.'
          )
          .setColor(0x00ff00),
      ],
      ephemeral: true,
  ...
  async removeChannel(interaction) {
    ...
    if (!channelId) {
      return interaction.reply({
        content: '❌ 이 서버에 타르코프 패치노트 알림이 설정되어 있지 않습니다.',
        ephemeral: true,
      });
    }
    ...
    await interaction.reply({
      content: '✅ 타르코프 패치노트 자동 알림이 해제되었습니다.',
      ephemeral: true,
    });
  },
